...
const char * const BITCOIN_CONF_FILENAME = "bitfishcoin.conf";
const char * const BITCOIN_PID_FILENAME = "bitfishcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitfishcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitfishcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitfishcoinCore
    // Mac: ~/Library/Application Support/BitfishcoinCore
    // Unix: ~/.bitfishcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitfishcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitfishcoinCore";
#else
    // Unix
    return pathRet / ".bitfishcoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty bitfishcoin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitfishcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Bitfishcoin Core developers";
    }
