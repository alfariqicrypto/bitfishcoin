...
# bitfishcoin_hash
RUN git clone https://github.com/Altcoin-Master/bitfishcoin_hash
RUN cd bitfishcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} bitfishcoin
RUN useradd -u ${USER_ID} -g bitfishcoin -s /bin/bash -m -d /bitfishcoin bitfishcoin
...
RUN mkdir /bitfishcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /bitfishcoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /bitfishcoin-src
