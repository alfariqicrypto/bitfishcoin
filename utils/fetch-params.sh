...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/BitfishcoinParams"
else
    PARAMS_DIR="$HOME/.bitfishcoin-params"
fi
...
DOWNLOAD_URL="https://github.com/Altcoin-Master/bitfishcoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with BTSC_DISABLE_SOMETHING=1
BTSC_DISABLE_WGET="${BTSC_DISABLE_WGET:-}"
#BTSC_DISABLE_IPFS="${BTSC_DISABLE_IPFS:-}"
BTSC_DISABLE_CURL="${BTSC_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$BTSC_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$BTSC_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$BTSC_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Bitfishcoin parameters!
...
main() {
    ...
    cat <<EOF
BTSC - fetch-params.sh

This script will fetch the Bitfishcoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Bitfishcoin SNARK parameters. Note that it is
